...
def test_cli_external_ref_warning(external_ref_spec_path: Path) -> None:
    """Verify external file references emit non-fatal warnings to stderr and metadata, with exit code 0."""
    runner = CliRunner()
    ...
    resp_500_ref = chunk["operation"]["responses"]["500"]["content"]["application/json"]["schema"]["$ref"]
    ...
    expected_warning = "External reference 'common_models.yaml#/components/schemas/ExternalError' is unsupported and was preserved without expansion."
...
def test_cli_external_ref_with_op(external_ref_spec_path: Path) -> None:
    """Verify --op spot check also preserves raw $ref and emits stderr warning for external references."""
    runner = CliRunner()

...
### 1.1 Command
Generate test cases for a sample spec containing path parameters and request bodies:
```bash
specprobe chunk tests/fixtures/petstore_sample.json | specprobe generate > /tmp/all_test_cases.jsonl
```
...
- Operations with path parameters (e.g., `GET /pets/{petId}`) produce:
  ...
  - 404 Not Found (`test_type: "negative_not_found"`, status 404, path parameter mutated to `999999`)
...
```bash
specprobe chunk tests/fixtures/petstore_sample.json | specprobe generate --no-not-found > /tmp/no_404_cases.jsonl
```
**Verification**:
```bash
python -c '
import json, sys
cases = [json.loads(line) for line in open("/tmp/no_404_cases.jsonl")]
...
```bash
specprobe chunk tests/fixtures/petstore_sample.json | specprobe generate --no-invalid-input > /tmp/no_400_cases.jsonl
```
**Verification**:
```bash
python -c '
import json, sys
cases = [json.loads(line) for line in open("/tmp/no_400_cases.jsonl")]
...
### 3.1 Postman Export
```bash
specprobe export /tmp/all_test_cases.jsonl --format postman --output /tmp/exported_postman.json
```
**Verification**:
- Inspect `/tmp/exported_postman.json`:
  - Contains items prefixed with `[404]` and `[400]`.
...
```bash
specprobe export /tmp/all_test_cases.jsonl --format http --output /tmp/exported_requests.http
```
**Verification**:
- Inspect `/tmp/exported_requests.http`:
